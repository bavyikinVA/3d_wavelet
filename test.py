...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.print_load_image.grid(row=2, column=0, pady=20, sticky="w")
        ...
        # self.entry_custom_scale = ctk.CTkEntry(self)
        # self.entry_custom_scale.grid(row=12, column=0, padx=45, sticky="w")
    ...
    def load_image_callback(self):
        ...
        if filename:
            ...
            self.load_button.configure(text="Загружено", text_color="black", fg_color="white", border_color="black",
                                       border_width=2)
            self.print_load_image.configure(text=image_path)
        else:
    ...
    def compute_wavelets(self, info_out):
        ...
        if info_out == 0:
            for scale in range(num_scale):
                ...
                plt.colorbar()
                plt.savefig(os.path.join(folder_path, f'График_расчетов_В_П_Масштаб_{scales[scale]}.png'))
                plt.close()
    ...
    def find_extremum(self):
        global scales, result, extremum_row_max_array, extremum_row_min_array, folder_path, num_scale, \
            extremum_col_min_array, extremum_col_max_array
        if self.row_checkbox.get() is True:
            if (self.max_checkbox.check_state is True) and (self.min_checkbox.check_state is False):
                for scale in range(num_scale):
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix, title='(максимум_по_строкам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix, title='(максимум_по_строкам)', scale=scales[scale],
            ...
            if (self.max_checkbox.check_state is False) and (self.min_checkbox.check_state is True):
                for scale in range(num_scale):
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix, title='(минимум_по_строкам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix, title='(минимум_по_строкам)', scale=scales[scale],
                                                   folder_path=folder_path)
            ...
            if (self.max_checkbox.check_state is True) and (self.min_checkbox.check_state is True):
                for scale in range(num_scale):
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix1, title='(минимум_по_строкам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix1, title='(минимум_по_строкам)', scale=scales[scale],
                                                   folder_path=folder_path)
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix2, title='(максимум_по_строкам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix2, title='(максимум_по_строкам)', scale=scales[scale],
                                                   folder_path=folder_path)
        ...
        if self.col_checkbox.check_state is True:
            if (self.max_checkbox.check_state is True) and (self.min_checkbox.check_state is False):
                for scale in range(num_scale):
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix3, title='(максимум_по_столбцам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix3, title='(максимум_по_столбцам)', scale=scales[scale],
                                                   folder_path=folder_path)
            ...
            if (self.max_checkbox.check_state is False) and (self.min_checkbox.check_state is True):
                for scale in range(num_scale):
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix4, title='(минимум_по_столбцам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix4, title='(минимум_по_столбцам)', scale=scales[scale],
                                                   folder_path=folder_path)
            ...
            if (self.max_checkbox.check_state is True) and (self.min_checkbox.check_state is True):
                for scale in range(num_scale):
                    ...
                    extremum_row_min_array.append(matrix5)
                    extremum_row_max_array.append(matrix6)
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix5, title='(минимум_по_столбцам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix5, title='(минимум_по_столбцам)', scale=scales[scale],
                                                   folder_path=folder_path)
                    ...
                    if (self.p_ex1_checkbox.check_state is True) and (self.p_ex2_checkbox.check_state is False):
                        self.save_extremum_txt(matrix6, title='(максимум_по_столбцам)', scale=scales[scale])

                    if (self.p_ex1_checkbox.check_state is False) and (self.p_ex2_checkbox.check_state is True):
                        self.save_extremum_picture(matrix6, title='(максимум_по_столбцам)', scale=scales[scale],
                                                   folder_path=folder_path)
    ...
    def save_extremum_txt(self, matrix, title, scale):
        global folder_path
        main_title = 'Точки_экстремумов_' + title + '_Масштаб_' + str(scale)
        matrix_str = '\n'.join(' '.join(map(str, row)) for row in matrix)
        file_path = os.path.join(folder_path, main_title + '.txt')

        with open(file_path, "w") as file:
            file.write(matrix_str)

    def save_extremum_picture(self, matrix, title, scale, folder_path):
